...
import { getIntelligenceIcon, useRotatingThemeIcon } from '../utils/themeUtils';
...
const SplashScreen = ({ onInitialized, progress = 0, onLanguageLoaded }: SplashScreenProps) => {
  ...
  // Get the saved theme for the splash screen
  const savedTheme = localStorage.getItem('intelligence-theme') || 'system';
  
  // Use the rotating icon hook with the saved theme
  const { containerProps, frontIconProps, backIconProps } = useRotatingThemeIcon(savedTheme);
    ...
    <div className="flex flex-col items-center justify-center h-screen w-screen bg-[var(--bg-primary)]">
      <div className="flex flex-col items-center">
        ...
        <div className="mb-6 animate-pulse">
          <div {...containerProps} style={{ width: '96px', height: '96px' }}>
            <img {...frontIconProps} className={`w-24 h-24 ${frontIconProps.className}`} />
            <img {...backIconProps} className={`w-24 h-24 ${backIconProps.className}`} />
          </div>
        </div>

        {/* App name */}
        <h1 className="text-4xl font-bold mb-8 text-[var(--text-primary)]">Intelligence</h1>
